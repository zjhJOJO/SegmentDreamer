...
from utils.shapee_utils import init_from_shapee
...
def readCircleCamInfo(path,opt):
    ...
    if not os.path.exists(ply_path):
        ...
        if opt.init_shape == 'sphere':
            ...
            xyz = xyz * 1.
            num_pts = xyz.shape[0]
        elif opt.init_shape == 'shapee':
            num_pts = int(num_pts/5000)
            xyz,rgb = init_from_shapee(opt.init_prompt)
            if xyz.shape[0] > 10000:
                rd_idx = np.random.choice(np.arange(1, xyz.shape[0]), size=4096, replace=False)
                xyz = xyz[rd_idx,:]
                rgb = rgb[rd_idx,:] 
            xyz[:,1] = - xyz[:,1]
            xyz[:,2] = xyz[:,2] + 0.15
