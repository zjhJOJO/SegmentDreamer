...
def set_logger(filename=None):
    ...
    # # set to clear the original content
    # with open(os.path.join('log', filename), 'w'):
    #     pass
    ...
    file_handler = logging.FileHandler(os.path.join('log', filename), mode='w') 
...
def adjust_text_embeddings(embeddings, azimuth, guidance_opt): # embedding list: len = 6
...
def guidance_setup(guidance_opt, **kwargs):
    guidance_type = kwargs.get('guidance_type')
    if guidance_type == "gcs":
    ...
    elif guidance_type == "sctd":
        from guidance.sctd_utils import StableDiffusion
        guidance = StableDiffusion(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
                        guidance_opt.t_range, guidance_opt.max_t_range, 
                        num_train_timesteps=guidance_opt.num_train_timesteps, 
                        ddim_inv=guidance_opt.ddim_inv,
                        ...
                        LoRA_path = guidance_opt.LoRA_path,
                        guidance_opt=guidance_opt,
                        multiphase=kwargs.get('multiphase'), # for sctd
                        num_ddim_timesteps=kwargs.get('num_ddim_timesteps'),
                        logger=kwargs.get('logger'),# for sctd
                        is_perp=guidance_opt.perpneg)
    elif guidance_type == "csd":
        from guidance.csd_utils import StableDiffusion
        guidance = StableDiffusion(guidance_opt.g_device, guidance_opt.fp16, guidance_opt.vram_O, 
                guidance_opt.t_range, guidance_opt.max_t_range, 
                num_train_timesteps=guidance_opt.num_train_timesteps, 
                ddim_inv=guidance_opt.ddim_inv,
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video, guidance_type, multiphase, num_ddim_timesteps, seed=None):
    first_iter = 0
    tb_writer, logger = prepare_output_and_logger(dataset, guidance_opt, guidance_type, multiphase=multiphase, num_ddim_timesteps=num_ddim_timesteps, seed=seed)
    ...
    guidance, embeddings = guidance_setup(guidance_opt, guidance_type=guidance_type, multiphase=multiphase, num_ddim_timesteps=num_ddim_timesteps, logger=logger)   
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        for i in range(C_batch_size):
            ...
            text_z = [embeddings['uncond']] # appending negative text embedding

            if guidance_opt.perpneg:
                text_z_comp, weights = adjust_text_embeddings(embeddings, azimuth, guidance_opt)
                text_z.append(text_z_comp) # text_z: list [uncond(1,77,1024), perp-neg(K,77,1024)]
...
def prepare_output_and_logger(args, guidance_opt, guidance_type, multiphase=None, num_ddim_timesteps=None, seed=None):    
    if not args._model_path:
        ...
        output_str = args.opt_path.split('/')[-1].split('.')[0] + "@" + str(guidance_opt.guidance_scale) + "_" + str(seed) + "_" + str(guidance_opt.t_threshold)
        ...
        if guidance_type == 'pcd':
            output_str += ('_' + str(multiphase) + '_' + str(guidance_opt.t_mode))
            
        # set logger
        logger = set_logger(output_str + '.log')
        
        # args._model_path = os.path.join("./output/", args.workspace, output_str)
        args._model_path = os.path.join("./outputs", args.workspace, output_str)
...
if __name__ == "__main__":
    ...
    # for phased consistency distillation
    parser.add_argument("--guid_scale", type=float, default=7.5,)
    parser.add_argument("--guidance_type", type=str, default='gcs',)
    parser.add_argument("--phase_num", type=int, default=4,)
    ...
    # modift the guidance scale
    gp.guidance_scale = args.guid_scale
    
    gaussians = training(lp, op, pp, gcp, gp, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.save_video, args.guidance_type, args.phase_num, args.num_ddim_timesteps, args.seed)
