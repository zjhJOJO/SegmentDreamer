...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, t_range=[0.02, 0.98], max_t_range=0.98, num_train_timesteps=None, 
                 ...
                 LoRA_path = None, guidance_opt=None, multiphase=4, num_ddim_timesteps=50, logger=None, is_perp=True):
        super().__init__()

        self.device = device
        self.precision_t = torch.float16 if fp16 else torch.float32
        self.is_perpneg = is_perp
        ...
        if is_safe_tensor:
            pipe = StableDiffusionPipeline.from_single_file(model_key, config=base_model_key, use_safetensors=True, torch_dtype=self.precision_t,)
            # load_safety_checker=False)
            # additional configure for civital models
            if guidance_opt.clip_skip == 2: 
                pipe.text_encoder = CLIPTextModel.from_pretrained(base_model_key, subfolder="text_encoder", num_hidden_layers=11, torch_dtype=self.precision_t)
        else:
        ...
        self.max_step_base = int(self.num_train_timesteps * t_range[1])
        self.warmup_step = int(self.num_train_timesteps*(max_t_range-t_range[1]))
        
        self.max_step = self.max_step_base + self.warmup_step
        ...
        # for sctd          
        self.edge_points = self.trajectory_segmentation(multiphase, guidance_opt, mode=guidance_opt.t_mode)
        
        self.logger = logger
        print(f'[INFO] loaded stable diffusion!')
        
    def trajectory_segmentation(self, num_segments, guidance_opt, mode="uniform"):
        if mode == "uniform":    
            sampled_indices = np.linspace(self.min_step, self.max_step, num=(num_segments + 1), endpoint=True) - 1
            edge_points = torch.flip(self.scheduler.timesteps, dims=(0,))[sampled_indices]
        if mode == "mono": # mononic increasing interval
            d = (2 * (self.max_step - self.min_step - num_segments * (guidance_opt.t_threshold - self.min_step))) // ((num_segments - 1) * num_segments)
            edge_points = [self.min_step] 
            for i in range(num_segments):
                next_point = edge_points[-1] + ((guidance_opt.t_threshold - self.min_step) + i * d)  # 计算下一个区间终点
                edge_points.append(next_point)   
                
            edge_points = torch.tensor(edge_points, device=self.device)
            edge_points[-1] = torch.tensor(self.max_step)
    ...
    @torch.no_grad()
    def cfg_cond_pred(self, latents_noisy, t, text_embeddings, resolution, guidance_scale, controlnet_cond=None,):
        latent_model_input = latents_noisy[None, :, ...].repeat(2, 1, 1, 1, 1).reshape(-1, 4, resolution[0] // 8, resolution[1] // 8, )
        tt = t.reshape(1, 1).repeat(latent_model_input.shape[0], 1).reshape(-1)
        latent_model_input = self.scheduler.scale_model_input(latent_model_input, tt[0])
        # being modified for controlnet 
    ...
    def train_step(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
        ...
        # using the fixed noise for x_e_0
        ...
        #+++++++++++++++++++++text embedding+++++++++++++++++++++++++++++
        uncond_embeddings, text_embeddings, negative_text_embeddings, text_embeddings_ = self.text_embeddings_preprocessing(embedding_inverse, text_embeddings, B, K)
        
        #+++++++++++++++++++++timestep selection scheduler+++++++++++++++++++++++++++++

        (t_t, t_s, t_e), alphas, sigmas = self.timestep_setting(warm_up_rate, mode=guidance_opt.t_mode, guidance_opt=guidance_opt)

        # Reverse ODE.
        with torch.no_grad():
            x_e_0 = self.scheduler.add_noise(latents, noise, t_e).to(self.precision_t)
            
            if t_t >= guidance_opt.t_threshold:
                # two steps ddim samping to get x_t
                x_s_e_null, _ = self.ode_inverse(t_e, t_s, x_e_0, uncond_embeddings, B)
                x_t_e_null, _ = self.ode_inverse(t_s, t_t, x_s_e_null, uncond_embeddings, B)
            else:
                # single step ddim samping to get x_t
                x_t_e_null, _ = self.ode_inverse(t_e, t_t, x_e_0, uncond_embeddings, B)
                
            #use CFG
            if use_control_net:
                pred_depth_input = pred_depth_input[None, :, ...].repeat(1 + K, 1, 3, 1, 1).reshape(-1, 3, 512, 512).half()
                eps_t_cond = self.cfg_cond_pred(x_t_e_null, t_t, text_embeddings, resolution, guidance_opt.guidance_scale, controlnet_cond=pred_depth_input)
            else:
                eps_t_cond = self.cfg_cond_pred(x_t_e_null, t_t, text_embeddings, resolution, guidance_opt.guidance_scale)
                
            # predict x_s from x_t with PF-ODE
            x_s_t_cond = self.f_solver.dpm_solver_first_order_update(eps_t_cond, t_t.reshape(1, 1).repeat(B, 1).reshape(-1), 
                                                                t_s.reshape(1, 1).repeat(B, 1).reshape(-1), x_t_e_null).to(self.precision_t)
            
            #++++++++++++ CD loss +++++++++++++
            x_e_s_null, _ = self.ode_inverse(t_s, t_e, x_s_t_cond, uncond_embeddings, B)
            x_e_t_null = x_e_0
            grad = x_e_s_null - x_e_t_null 
            #++++++++++++ guidance loss +++++++++++++
            x_e_t_cond = self.f_solver.dpm_solver_first_order_update(eps_t_cond, t_t.reshape(1, 1).repeat(B, 1).reshape(-1), 
                                                                t_e.reshape(1, 1).repeat(B, 1).reshape(-1), x_t_e_null).to(self.precision_t)
            
            grad += (guidance_opt.guidance_scale + 1) * (x_e_t_null - x_e_t_cond)
            
        # Anomaly Detectionz
        assert not torch.isnan(grad).any(), f"NaN detected in grad at iteration {iteration}, t_t: {t_t}, t_s: {t_s}, t_e: {t_e}"
            
        # calculate loss
        w = lambda alphas: (((1 - alphas) / alphas) ** 0.5)
        w_t = w(self.alphas[t_t])
        
        grad = torch.nan_to_num(grad_scale * w_t * grad)
        loss = SpecifyGradient.apply(latents, grad)

        #+++++++++ pixelwise constraint +++++++++
        if guidance_opt.w_cp > 0.:
            with torch.no_grad():
                eps_e_cond = self.cfg_cond_pred(x_e_t_cond, t_e, text_embeddings, resolution, guidance_opt.guidance_scale)
                eps_e_null_ = self.cond_pred(x_e_t_null, t_e, uncond_embeddings)
                
                x_0_e_cond = pred_original(self.scheduler, eps_e_cond, t_e, x_e_t_cond)
                x_0_e_null = pred_original(self.scheduler, eps_e_null_, t_e, x_e_t_null)
                
                pred_x0_e_cond = self.decode_latents(x_0_e_cond)
                pred_x0_e_null = self.decode_latents(x_0_e_null)
                pred_div = (pred_rgb + pred_x0_e_cond - pred_x0_e_null)

            L_cp = guidance_opt.rgb_scale * guidance_opt.w_cp * (torch.nn.functional.mse_loss(pred_rgb, pred_div, reduction="sum") / B).to(self.precision_t)
            loss += torch.nan_to_num(L_cp)
            
        #+++++++++ log +++++++++
        if guidance_opt.w_cp > 0.:
            self.logger.info(f'Iteration: {iteration}, t_t: {t_t}, t_s: {t_s}, t_e: {t_e}, diff: {t_t-t_e}, grad: {grad.norm().item()}, w: {w_t.item()}, l_cp: {L_cp.item()}')
        else:
            self.logger.info(f'Iteration: {iteration}, t_t: {t_t}, t_s: {t_s}, t_e: {t_e}, diff: {t_t-t_e}, grad: {grad.norm().item()}, w: {w_t.item()}')
        ...
        if iteration % guidance_opt.vis_interval == 0:
            ...
            save_path_iter = os.path.join(save_folder,"iter_{}_step_{}_{}_{}.jpg".format(iteration,t_t.item(), t_s.item(), t_e.item()))
            with torch.no_grad():
                if guidance_opt.w_cp > 0.:
                    pred_x0_latent_t = x_0_e_cond
                    pred_x0_latent_e = x_0_e_null
                    pred_x0_s_p = self.decode_latents(x_0_e_cond)
                    pred_x0_s_n = self.decode_latents(x_0_e_null)
                    pred_x0_pos = pred_x0_s_p
                    pred_x0_sp  = pred_x0_s_n
                    grad = (pred_x0_s_p - pred_x0_s_n)
                ...
                viz_images = torch.cat(vis_list, dim=0) 
    ...
    def train_step_perpneg(self, text_embeddings, pred_rgb, pred_depth=None, pred_alpha=None,
        ...
        weights = weights.reshape(-1) # weights for perp-neg
        
        # using the fixed noise for x_e_0
        if self.noise_temp is None:
            self.noise_temp = torch.randn((latents.shape[0], 4, resolution[0] // 8, resolution[1] // 8, ), dtype=latents.dtype, device=latents.device, generator=self.noise_gen) + 0.05 * torch.randn((1, 4, 1, 1), device=latents.device).repeat(latents.shape[0], 1, 1, 1)
        ...
        with torch.no_grad():
            ...
            # whether to use perp-neg
            eps_t_perp = self.cfg_perpneg_pred(x_t_e_null, t_t, text_embeddings, weights, guidance_opt.guidance_scale, K, B, resolution)
    
            # predict x_s from x_t with PF-ODE
            x_s_t_cond = self.f_solver.dpm_solver_first_order_update(eps_t_perp, t_t.reshape(1, 1).repeat(B, 1).reshape(-1), 
                                                                t_s.reshape(1, 1).repeat(B, 1).reshape(-1), x_t_e_null).to(self.precision_t)
            ...
            grad = x_e_s_null - x_e_t_null        
            #++++++++++++ guidance loss +++++++++++++
            x_e_t_cond = self.f_solver.dpm_solver_first_order_update(eps_t_perp, t_t.reshape(1, 1).repeat(B, 1).reshape(-1), 
                                                                t_e.reshape(1, 1).repeat(B, 1).reshape(-1), x_t_e_null).to(self.precision_t)
        ...
        if guidance_opt.w_cp > 0.:
            with torch.no_grad():
                eps_e_perp = self.cfg_perpneg_pred(x_e_t_cond, t_e, text_embeddings, weights, guidance_opt.guidance_scale, K, B, resolution)
                eps_e_null_ = self.cond_pred(x_e_t_null, t_e, uncond_embeddings)
                
                x_0_e_perp = pred_original(self.scheduler, eps_e_perp, t_e, x_e_t_cond)
                x_0_e_null = pred_original(self.scheduler, eps_e_null_, t_e, x_e_t_null)
                
                pred_x0_e_perp = self.decode_latents(x_0_e_perp)
                pred_x0_e_null = self.decode_latents(x_0_e_null)
                pred_div = (pred_rgb + pred_x0_e_perp - pred_x0_e_null)
        ...
        if iteration % guidance_opt.vis_interval == 0:
            ...
            with torch.no_grad():
                if guidance_opt.w_cp > 0.:
                    pred_x0_latent_t = x_0_e_perp
                    pred_x0_latent_e = x_0_e_null
                    pred_x0_s_p = self.decode_latents(x_0_e_perp)
                    pred_x0_s_n = self.decode_latents(x_0_e_null)
                    pred_x0_pos = pred_x0_s_p
                    pred_x0_sp  = pred_x0_s_n
                    grad = (pred_x0_s_p - pred_x0_s_n)
    ...
    def timestep_setting(self, warm_up_rate, mode="uniform", guidance_opt=None, t_eps=10):
        
        # timestep ~ U(0.02, 0.98) to avoid very high/low noise level
        ind_t = torch.randint((self.min_step + int(self.warmup_step * warm_up_rate)), (self.max_step_base + int(self.warmup_step * warm_up_rate)), (1, ), 
                              dtype=(torch.long), generator=(self.noise_gen), device=(self.device))[0]
        t_t = self.timesteps[ind_t]

        # if mode == "uniform":
        delta_ = guidance_opt.delta_t
        resi = t_t - self.edge_points
        min_idx = torch.argmin(resi[resi >= 0 ])
    
        t_e = self.edge_points[min_idx]
        t_t = torch.max(t_t, t_e + delta_)
        t_s = torch.randint((t_e + t_eps), (t_t - t_eps), (1, ), dtype=(torch.int64), generator=(self.noise_gen), device=(self.device))[0]

        
        # obtain alpha and sigma
        alpha_t, alpha_s, alpha_e = self.alpha_schedule[t_t-1], self.alpha_schedule[t_s-1], self.alpha_schedule[t_e-1]
        sigma_t, sigma_s, sigma_e = self.sigma_schedule[t_t-1], self.sigma_schedule[t_s-1], self.sigma_schedule[t_e-1]
        
        return (t_t, t_s, t_e), (alpha_t, alpha_s, alpha_e), (sigma_t, sigma_s, sigma_e)
    
    def compute_weights(self, alphas, sigmas,):
        lambda_t = torch.log(alphas[0] / sigmas[0])
        lambda_e = torch.log(alphas[-1] / sigmas[-1])
        return 1. / (alphas[-1] * (torch.exp(-lambda_t) - torch.exp(-lambda_e)))
        
    def text_embeddings_preprocessing(self, embedding_inverse, text_embeddings, B, K):
        ...
        negative_text_embeddings = text_embeddings[:4]
        text_embeddings_ = torch.cat([uncond_embeddings, text_embeddings[4:]], dim=0)

        return uncond_embeddings, text_embeddings, negative_text_embeddings, text_embeddings_
