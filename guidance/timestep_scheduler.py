...
class TimePrioritizedScheduler:
    def __init__(self, cfg, scheduler, device, num_train_timesteps=1000) -> None:
        
        # Hyper Params
        self.device = device

        self.time_sampling = cfg.time_sampling #['uniform', 'linear', 'annealed']
        # self.min_step = int(num_train_timesteps * cfg.min_timestep) # 0.020
        # self.max_step = int(num_train_timesteps * cfg.max_timestep) # 0.098
        min_timestep_ratio = cfg.t_range[0]
        max_timestep_ratio = cfg.max_t_range
        self.time_prior = cfg.time_prior # "uniform"

        self.min_step = int(num_train_timesteps * min_timestep_ratio) # 0.020
        self.max_step = int(num_train_timesteps * max_timestep_ratio) # 0.098
    ...
    def draw_curves(self, save_dir):
        ...
        if self.time_sampling == 'annealed':
            ...
            plt.legend()
            plt.savefig(os.path.join(save_dir,'tp_weights.png'))
            plt.clf()
        ...
        plt.savefig(os.path.join(save_dir,'tp_i2t.png'))
        plt.clf()
...
if __name__ == "__main__":
    ...
    config_pth = '/media/data1/zhujh/ECCV2024-GCS-BEG/configs/full_model/tiger.yaml'
    model_key = '/media/data1/zhujh/sd_ckpt/stable-diffusion-2-1-base'
    save_dir = '/media/data1/zhujh/ECCV2024-GCS-BEG/output/temp'
    device = 'cpu'

    conf = OmegaConf.load(config_pth)
    schedule = DDIMScheduler.from_pretrained(model_key,  subfolder="scheduler", torch_dtype=torch.float16)
    ts = TimePrioritizedScheduler(conf.GuidanceParams, schedule, device)
    ts.draw_curves(save_dir)
